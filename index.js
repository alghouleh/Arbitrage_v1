...
if (APIKEY==='' || APIKEY==='') {
  console.log("Please input your Binance API info.")
  process.exit()
}

const profit_threshold = 0.35
...
async function listenToAccount() {
  ...
  console.log('Time to retrieve exchangeInfo:', Date.now() - timer, 'ms')
  ...
  const clean = await bnb_client.ws.user( async msg => {
    ...
    if (msg.eventType === 'executionReport' && msg.orderStatus === 'FILLED' && msg.symbol === asset_1_pair) {
      console.log("executionReport 1", msg.side, msg.symbol, msg.quantity, msg.totalQuoteTradeQuantity)
      asset_1_pair = ''
      ...
      console.log("executionReport 2", msg.side, msg.symbol, msg.quantity, msg.totalQuoteTradeQuantity)
      asset_2_pair = ''
      ...
      console.log("executionReport 3", msg.side, msg.symbol, msg.quantity, msg.totalQuoteTradeQuantity)
      asset_3_pair = ''
      order_status = 0
      console.log("END.", msg.symbol, msg.quantity)
      busy = false
      console.log('Time to process the orders:', Date.now() - processTimer, 'ms')
      //process.exit()
    }
...
arbitrage.eventEmitter.on('ARBITRAGE', async (pl) => {
  try {
  if ( pl[0].value > 0.1 ) console.log(pl[0].value, pl[0].tpath, Date.now()-pl[0].timen+' ms')
...
}catch(err){console.log('No Profitable Trades')}
})
